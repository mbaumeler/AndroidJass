...
class HTTPSession implements Runnable {
	 ...
	 * 
	 */
	private final NanoHTTPD nanoHttp;

	public HTTPSession(NanoHTTPD nanoHttp, Socket s) {
		this.nanoHttp = nanoHttp;
		mySocket = s;
		...
		try {
			...
			if (method.equalsIgnoreCase("POST")) {
				...
				if (contentType.equalsIgnoreCase("multipart/form-data")) {
					...
					if (!st.hasMoreTokens())
						sendError(NanoHTTPD.HTTP_BADREQUEST,
			...
			// Ok, now do the serve()
			Response r = this.nanoHttp.serve(uri, method, header, parms, files);
			if (r == null)
	...
	private void sendResponse(String status, String mime, Properties header, InputStream data) {
		try {
			...
			if (data != null) {
				...
				while (pending > 0) {
					int read = data.read(buff, 0, ((pending > NanoHTTPD.theBufferSize) ? NanoHTTPD.theBufferSize
							: pending));
					if (read <= 0)
						break;
